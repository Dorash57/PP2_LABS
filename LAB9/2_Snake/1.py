...
pygame.display.set_caption("Snake with Levels and Food Timer")
...
# Рисуем шахматную сетку (не используется в основной логике, но можно подключить)
...
class Food:
    ...
    def is_expired(self):
        return time.time() - self.spawn_time > 5  # исчезает через 5 секунд
...
while running:
    ...
    # Проверка на истечение времени еды
    if food.is_expired():
        food.generate_random_pos(snake.body)
